...
import getOfferApplications from "../../graphql/queries/getOfferApplications";
...
class OfferApplicationTab extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  renderItem = (item, key) => {
    let url = link.url + "/uploads/profile_images/" + item.profile_photo;
    return (
      <View key={key} style={styles.threadContainer}>
        ...
        >
          {item.profile_photo ? (
            ...
            <Avatar.Text
              style={{ marginRight: 15 }}
              size={50}
              label={firstChar(item.firstname) + firstChar(item.lastname)}
            />
          )}
        ...
        {item.pivot.direction === "you" ? (
          item.pivot.status === "0" ? (
            ...
            >
              <Text style={styles.notAcceptBtnText}>Approved</Text>
            </View>
          )
        ) : null}
      </View>
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          <ScrollView>
            ...
            >
              Following is the all Applications
            </Text>
            <Query
              query={getOfferApplications}
            ...
            >
              {({ loading, error, data, fetchMore, refetch }) => {
                ...
                if (data.offer.applications.length > 0) {
                  return data.offer.applications.map((item, key) => {
...
export default OfferApplicationTab;
